...
import { verifyReportSchema } from '@/schemas/admin-report-schema'
...
export async function PATCH(request: Request, context: VerifyRouteContext): Promise<Response> {
  try {
    ...
    if (!canTransitionReportStatus(report.status, ReportStatus.VERIFIED)) {
    ...
    const updatedReport = await prisma.$transaction(async (tx) => {
      ...
      await tx.notification.create({
        data: {
          ...
          title: 'reports.notifications.reportVerified.title',
          message: 'reports.notifications.reportVerified.message'
        }
      })
    ...
    return successResponse(data, 'reports.admin.messages.verifySuccess')
  } catch {
